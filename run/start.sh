...
myclone ${GIT_REPO_SYNC}  ${STORE_BOLT_PATH} || mkdir -p ${STORE_BOLT_PATH}
[[ -z "$GIT_REPO_BACKUP" ]] || myclone ${GIT_REPO_BACKUP} "$BACKUP_PATH" 
mkdir -p ${STORE_BOLT_PATH}
( sleep 10; [[ -z "$SLEEPINTER" ]]   &&  SLEEPINTER=90; while (true);do ( cd ${STORE_BOLT_PATH} ; git add -A  ;git commit -m $(date +%F_%T)"auto"; mypush ) ; [[ -z "$GIT_REPO_BACKUP" ]] || ( cd "$BACKUP_PATH" ; git add -A  ;git commit -m $(date +%F_%T)"auto" ;mypush )  ; sleep $SLEEPINTER ; done ) &  
