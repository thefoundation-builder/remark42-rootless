...
( sleep 10; [[ -z "$SLEEPINTER" ]]   &&  export SLEEPINTER=90; while (true);do ( cd ${GITPATH} ; pwd; mypush ) ; [[ -z "$GIT_REPO_BACKUP" ]] || ( cd "$BACKUP_PATH" ; git add -A  ;git commit -m $(date +%F_%T)"auto" ;mypush )  ; sleep $SLEEPINTER ; done ) &  
