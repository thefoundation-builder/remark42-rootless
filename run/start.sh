...
while (true);do 
##att multiline ahead
  MAIL_NO_TLS=true MAIL_FROM=mails@webmention-ui.local MAIL_PORT=1025 EMAIL_HOST=127.0.0.1 MAIL_HOST=127.0.0.1   SERVER_AUTH_JWT_SECRET=$JWTSECRET /usr/local/bin/webmentiond serve    --public-url=$URL/webmentions       --allowed-target-domains "mydomain.lan"    --auth-admin-emails "admina@abc.de"        --database-migrations /var/lib/webmentiond/migrations    --database /srv/webmentiond.sqlite     --verification-timeout=120s    --verification-max-redirects=5
